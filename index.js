...
function getData3(URL){
  ...
  .done(function(){
    var dataR = data.responseJSON.data;
    for (var i=0;i<dataR.length;i++){
      houses.push([dataR[i][15],dataR[i][19],parseInt(dataR[i][33])]);
      if(estoy==false){
        housings.push([dataR[i][15],dataR[i][19],parseInt(dataR[i][33])]);
        estoy=true;
      }
var toy = false;
      for (var h = 0; h < housings.length; h++){
          if(dataR[i][19]==housings[h][1]){
          housings[h][2]= (parseInt(housings[h][2]) + parseInt(dataR[i][33]));
          toy = true;
      }
    }
    if(toy==false){
...
//console.log(tabla3);
  for (var i = 0; i < housings.length; i++) {
    ...
    var newR,borough,borDist,lows;
    for(var j=0;j<housings.length;j++){
      newR = tableReference.insertRow(tableReference.rows.length);
      borough = newR.insertCell();
      borDist = newR.insertCell();
      lows = newR.insertCell();
      borough.innerHTML = housings[j][0];
      borDist.innerHTML = housings[j][1];
      lows.innerHTML    = housings[j][2];
    }
    console.log(infoRows);
  })
  .fail(function error(){
